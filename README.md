...
* [sbakhour/freshdep: Lightweight, dependency-free CLI that audits an existing `requirements.txt`/`package-lock.json` for pinned dependencies published more recently than a freshness threshold, as a supply-chain compromise tripwire -- read-only, no install-time wrapping](https://github.com/sbakhour/freshdep)

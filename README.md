...
* [skills-lock/skil-lock: Lockfile + GitHub Action that pins the parsed behavior surface (shell commands, URLs, file reads/writes, allowed tools) of Claude Code and Codex agent skills. Catches "SKILL.md legitimately changed and now does something different" between PRs that hash pinning misses. Outputs SARIF for GitHub Code Scanning. Apache 2.0](https://github.com/skills-lock/skil-lock)

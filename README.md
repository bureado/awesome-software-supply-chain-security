...
* [matteo-sung/lockvet: Explains and gates lockfile and manifest changes before merge — release-verifies every bump against 20 registries and flags OSV/malware advisories, versions missing from the registry index, typosquats, deprecations, license changes and same-version integrity/resolution tampering across 46 formats (npm, PyPI, Cargo, Go incl. go.sum, Maven, GitHub Actions pins, pre-commit configs, Dockerfiles, Kubernetes/Flux/Argo image and chart pins, Helm charts and values, SBOMs); single Go binary with CI gate, SARIF, MCP server and an in-browser playground; vets PRs by URL on GitHub, GitLab, Bitbucket, Gitea/Forgejo and Azure DevOps](https://github.com/matteo-sung/lockvet)

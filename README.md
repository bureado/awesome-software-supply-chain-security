...
* [piiiico/proof-of-commitment: CLI, MCP server and GitHub Action that scores npm, PyPI, Cargo and Go packages on behavioral commitment signals — publisher depth (single-publisher = CRITICAL), release consistency, maintenance patterns — surfacing supply-chain risk that stars and download counts miss. Scans direct + transitive deps from lockfiles](https://github.com/piiiico/proof-of-commitment). Try: [getcommit.dev](https://getcommit.dev)

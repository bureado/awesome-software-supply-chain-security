...
* [rccaoki-wq/license-guard: Evaluates a declared dependency license against the way the software is shipped — hosted service, distributed binary, on-prem delivery, internal only, or published library — and cites the clause that produced the verdict](https://github.com/rccaoki-wq/license-guard), on the basis that copyleft obligations attach to conveying or network interaction rather than to the presence of the code, so GPL, AGPL §13, dev-only dependencies and LGPL static vs dynamic linking do not collapse into one risk column. Reads npm, PyPI, Go and crates.io manifests and lockfiles; available as an MCP server so the check happens when a dependency is added
